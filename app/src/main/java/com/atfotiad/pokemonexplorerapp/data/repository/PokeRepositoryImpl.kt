...
class PokeRepositoryImpl @Inject constructor(
    ...
    override val totalPokemonCount = _totalPokemonCount.asStateFlow()


    override fun getPokemonByName(name: String): Flow<Result<Pokemon>> = flow {
        emit(Result.Loading)
    ...
    override fun getPokemonList(offset: Int, limit: Int): Flow<Result<List<Pokemon>>> = flow {
        emit(Result.Loading)
    ...
    private suspend fun fetchSpeciesAndCreatePokemon(pokemonResponse: PokemonResponse?): Result<Pokemon> {
        return pokemonResponse?.let {
            ...
            println("Extracted speciesId: $speciesId") // Debug log
            val speciesEntryResult = remoteDataSource.getSpeciesEntry(speciesId)

            return if (speciesEntryResult is Result.Success) {
                val flavorText = speciesEntryResult.data?.flavorTextEntries?.firstOrNull { entry -> entry.language.name == "en" }?.flavorText ?: ""
                ...
                speciesEntryResult as Result<Pokemon>
            }
        } ?: Result.Success(null)
    }
}